...
AutoFlow uses **UMAP** for visualisation (3D where available).

### Clustering
Nearest-neighbors graph → **Louvain/Leiden** clustering with adjustable resolution.

### Markers / Auto-annotation (Unsupervised)
Differential expression (**Wilcoxon rank sum**) can be used to label clusters with a compact “marker string” (e.g., `CD34+CD38-`).

### Fast Unsupervised Mode (Subsample + Project)
For large datasets, AutoFlow can run a faster unsupervised workflow:

1. **Subsample training cells** (default max: `100000`) from the full dataset.
2. Train **PCA → UMAP → neighbors → clustering** on the training subset.
3. **Project** the learned UMAP model onto all remaining cells.
4. Transfer `seurat_clusters` / `assignment` to the remaining cells using **k-nearest neighbours (kNN) in PCA space**.

This reduces runtime while keeping embeddings/clusters comparable to the full run in most cases (users should validate on their own data).
...
When preprocessing is enabled, AutoFlow also produces a **QC summary table** per file (events before/after transform and counts flagged as debris/doublet/badqc). This can be downloaded for record-keeping and debugging.

**Files**
- **Processed zip of .csv files per uploaded file** (optional) with:
  - Labels for QC flags (debris/doublet/badqc/viability).
