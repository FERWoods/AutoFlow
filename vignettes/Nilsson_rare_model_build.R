...
nilsson <- read.FCS(file.path(dir, "Nilsson_rare.fcs"), transformation = NULL)
X_all   <- exprs(nilsson)                     # numeric matrix [events x channels]
...
drop_cols <- c("FSC-A","FSC-W","FSC-H","SSC-A","Time")
...
saveRDS(bundle, file.path(outdir, "autoflow_nilsson_ranger_bundle.rds"))
...
ff_train <- make_subset_ff(nilsson, train_idx)
ff_test  <- make_subset_ff(nilsson, setdiff(seq_len(nrow(X_all)), train_idx))

write.FCS(ff_train, file.path(outdir, "Nilsson_rare_TRAIN.fcs"))
write.FCS(ff_test,  file.path(outdir, "Nilsson_rare_TEST.fcs"))

cat("Wrote:\n",
    file.path(outdir, "Nilsson_rare_TRAIN.fcs"), "\n",
    file.path(outdir, "Nilsson_rare_TEST.fcs"),  "\n")
