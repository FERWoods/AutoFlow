...
app_server <- function(input, output, session) {
  ...
  files_all <- reactive({
    ...
    if(!is.integer(input$files) & input$model_type == "Unsupervised"){
      ...
      plot(cluster_dat@reductions$umap@cell.embeddings[,1],
           cluster_dat@reductions$umap@cell.embeddings[,2],
           col = factor(cluster_dat$proliferation), pch=".")
      table(cluster_dat$assignment, cluster_dat$proliferation)
      FeaturePlot(cluster_dat, features=rownames(cluster_dat))
      #out_dat <<- files_all()
      return(cluster_dat)
